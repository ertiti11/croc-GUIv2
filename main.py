...
def ejecutar_comando_recibir_thread(comando):
    proceso = subprocess.Popen(
        comando, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
    )
...
def ejecutar_comando(comando, buscar_codigo=False):
    ...
    for linea in iter(proceso.stdout.readline, ""):
        resultado_label.after(
            0, resultado_label.configure, {"text": resultado_label.cget("text") + linea}
        )
...
# image_label = tk.CTkLabel(root, image=my_image, text="")
...
recibir_comando_btn = tk.CTkButton(
    recibir_frame, text="Recibir", command=ejecutar_comando_recibir
)
recibir_comando_btn.pack(pady=10)
copiar_codigo_btn = tk.CTkButton(
    recibir_frame,
    text="Copiar código al portapapeles",
    command=lambda: copy_to_clipboard(resultado_label.cget("text")),
)
...
enviar_btn = tk.CTkButton(
    enviar_frame,
    text="Enviar",
    command=lambda: ejecutar_comando_enviar(text_entry.get()),
)
enviar_btn.pack(pady=10)
copiar_codigo_btn = tk.CTkButton(
    enviar_frame, text="Copiar código al portapapeles", command=copy_to_clipboard
)
copiar_codigo_btn.pack(pady=10)
